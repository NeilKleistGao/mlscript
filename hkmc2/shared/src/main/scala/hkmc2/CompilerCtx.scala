  ...
  def getElaboratedBlock
    ...
    def mk =
      ...
      val parse =
        ...
        ParserSetup(file)
      // Compile every unit in its own scope above the shared prelude. Besides
      // preventing a unit from mutating the cached prelude environment, this
      // lets explicit imports and opens shadow prelude names locally.
      given Elaborator.Ctx = prelude.nestLocal("compilation-unit")
      ...
      val blk = new Term.Blk(
        Import(State.runtimeSymbol, paths.runtimeFile.toString, paths.runtimeFile) ::
          // Only import staging support and `Term.mls` when necessary.
          (if hasQuote then
            Import(State.termSymbol, paths.termFile.toString, paths.termFile) :: Nil
          else
            Nil) :::
          (if isStaged then
            Import(State.optionSymbol, paths.optionFile.toString, paths.optionFile) ::
            Import(State.shapeSetSymbol, paths.shapeSetFile.toString, paths.shapeSetFile) ::
            Import(State.blockSymbol, paths.blockFile.toString, paths.blockFile) ::
            Import(State.specializeHelpersSymbol, paths.specializeHelpersFile.toString, paths.specializeHelpersFile) :: Nil
          else
            Nil) ::: blk0.stats,
        blk0.res
      )
