        ...
        case AssignField(l @ Value.This(cls), Tree.Ident(n), r, rest) if cls == defn.isym =>
          privateFields.get(n) match
            case S((b, _, publicField)) =>
              applyResult(r): r =>
                r match
                  case p: Path => Define(ValDefn(publicField, b, p)(N, Nil), applyBlock(rest))
                  case r => assign(r): p =>
...
class ReflectionInstrumenter(using State, Raise, Ctx) extends BlockTransformer(SymbolSubst.Id):
  ...
  private case class GeneratedLambdaCtor(fun: FunDefn, cls: ClsLikeDefn)
  private val generatedLambdaCtors = HashMap.empty[Symbol, GeneratedLambdaCtor]
  private val stagedLambdaClasses = HashMap.empty[Symbol, ClsLikeDefn]
  private val stagedModuleLambdaFunctions = HashMap.empty[InnerSymbol, Ls[FunDefn]]
  private val stagedLambdaFunctionNames = HashMap.empty[Symbol, Str]
  private val stagedLambdaFunctionOwners = HashMap.empty[Symbol, InnerSymbol]
  ...
  def transformSymbol(sym: Symbol, preserveName: Bool, pOpt: Option[Path], symName: Str)(using ctx: Context)(k: (Path, Context) => Block): Block =
    ...
    ctx.getCache(sym).map(cachedK(_, ctx)).getOrElse:
      ...
      val generatedStagedClass = sym match
        case c: ClassSymbol => defnMap.get(c).exists(isStagedLambdaClass)
        case _ => false
      val rename = sym match
        case _ if pOpt.isDefined => false
        case _ if preserveName => scope.allocateOrGetName(sym); false
        case _ if generatedStagedClass => true
        ...
        case _ => false
      val name = stagedLambdaFunctionNames.get(sym).getOrElse:
        if rename then
          val allocated = scope.allocateOrGetName(sym)
          if generatedStagedClass then allocated.replace("$", "Closure") else allocated
        else sym.nme
      sym match
              ...
              transformParamsOpt(paramsOpt): (paramsOpt, ctx) =>
                auxParams.map(ps => ctx => transformParamList(ps, Nil)(using ctx)).chainContext: (auxParams, ctx) =>
                  tuple(auxParams): auxParams =>
                    val redirect = rename && !generatedStagedClass
                    blockCtor("ConcreteClassSymbol", Ls(toValue(name), path, paramsOpt, auxParams, toValue(redirect)), symName)(checkMap("checkClassMap", path, _, ctx))
    ...
    ctx.getCache(p).map(k(_, ctx)).getOrElse:
      p match
        ...
        case Value.MemberRef(bms, disamb) =>
          val owner = stagedLambdaFunctionOwners.get(bms).orElse(stagedLambdaFunctionOwners.get(disamb))
          val name = stagedLambdaFunctionNames.get(bms).orElse(stagedLambdaFunctionNames.get(disamb))
          (owner, name) match
            case (S(owner), S(name)) =>
              transformPath(Select(Value.This(owner), Tree.Ident(name))(N)(false))(k)
            case _ =>
  ...
  def stageMethod(f: FunDefn, ctx: Context = Context(false)): FunDefn =
    val stageSymName = stagedFunctionName(f) + "_instr"
  ...
  def genMethod(cache: Path, classFun: Bool)(f: FunDefn, stagedPath: Path) =
    val funName = stagedFunctionName(f)
    val genSymName = funName + "_gen"
    ...
    val body = params.map(ps => tuple(ps.params.map(_.sym))).collectApply: tups =>
      tuple(tups): args =>
        call(helperMod("specialize"), Ls(cache, toValue(funName), stagedPath, args)): res =>
          Return(res)
  ...
  def stageMethods(cfg: StagingCfg)(methods: Ls[FunDefn]): (FunDefn, Ls[FunDefn], Block => Block) =
    ...
    val (stagedMethods, generatorMethods, generatorEntries) = methods.map(f =>
      ...
      (
        staged,
        gen,
        tuple(Ls(toValue(stagedFunctionName(f)), modSym.asPath.selSN(gen.sym.nme)))
      )
    ).unzip3
  ...
  override def applyObjBody(companion: ClsLikeBody) =
    if companion.isStaged then
      ...
      class UsedStagedClassesCollector extends BlockTraverser:
        ...
        val generatedLambdaClasses: HashSet[ClsLikeDefn] = new HashSet()
        override def applySymbol(sym: Symbol) = sym match
          case sym if generatedLambdaCtors.contains(sym) =>
            val cls = generatedLambdaCtors(sym).cls
            generatedLambdaClasses += stagedLambdaClasses.getOrElse(cls.isym, cls)
          case sym if stagedLambdaClasses.contains(sym) =>
            generatedLambdaClasses += stagedLambdaClasses(sym)
      ...
      val generatedLambdaClasses = collector.generatedLambdaClasses.toList.sortBy(_.sym.uid)
      val reflectedCtor = generatedLambdaClasses.foldRight(ctor):
        case (cls, rest) => Define(cls, rest)
      val ctorFun = FunDefn.withFreshSymbol(S(modSym), BlockMemberSymbol("ctor$", Nil, false), Ls(PlainParamList(Nil)), reflectedCtor)(N, Nil)
      ...
      val generatedLambdaPropagates = generatedLambdaClasses.map: cls =>
        cls.sym.asMemberRef(cls.isym)
      val cfg = new StagingCfg(
        companion.isym,
        modSym,
        nestedPropagates ++ generatedLambdaPropagates,
        codegenClasses.toList,
      )
      val lambdaFunctions = stagedModuleLambdaFunctions.getOrElse(companion.isym, Nil)
      val (entryFun, newMethods, cont) = stageMethods(cfg)(methods ++ lambdaFunctions)
  ...
  private def isGeneratedLambdaClass(defn: ClsLikeDefn): Bool =
    defn.parentPath.exists(_ == firstClassFunc) && !defn.sym.nameIsMeaningful

  private def isStagedLambdaClass(defn: ClsLikeDefn): Bool =
    stagedLambdaClasses.contains(defn.sym) || stagedLambdaClasses.contains(defn.isym)

  private def markStaged(fun: FunDefn): FunDefn =
    if fun.isStaged then fun
    else fun.copy()(fun.configOverride, Annot.Modifier(Keyword.`staged`) :: fun.annotations)

  private def stagedFunctionName(fun: FunDefn): Str =
    stagedLambdaFunctionNames.getOrElse(fun.sym, fun.sym.nme)
  ...
  override def applyBlock(b: Block): Block = b match
    ...
    case Define(defn: ClsLikeDefn, rest) if isStagedLambdaClass(defn) && !defn.privateFields.isEmpty =>
      (new DataClassTransformer).applyClsLikeDefn(defn): defn =>
        defn match
          case cls: ClsLikeDefn =>
            val reflectedCls = (cls.paramsOpt, cls.auxParams) match
              case (N, params :: rest) => cls.copy(paramsOpt = S(params), auxParams = rest)(cls.configOverride, cls.annotations)
              case _ => cls
            stagedLambdaClasses.addOne(cls.sym -> reflectedCls)
            stagedLambdaClasses.addOne(cls.isym -> reflectedCls)
            defnMap.addOne(cls.isym -> reflectedCls)
            applyBlock(Define(cls, rest))
          case _ => die
    // staged classes
    case Define(defn: ClsLikeDefn, rest) if defn.isStaged || isStagedLambdaClass(defn) =>
      ...
      def replaceSuper(parentPath: Path) = new BlockTransformer(SymbolSubst.Id):
        override def applyResult(r: Result)(k: Result => Block) = super.applyResult(r):
          ...
          case r => k(r)
      val preCtor =
        if isStagedLambdaClass(defn) then End()
        else defn.parentPath match
  ...
  // * Find generated closure classes referenced by staged modules before instrumentation starts.
  private def collectStagedLambdaClasses(b: Block): Unit =
    val defns = ListBuffer.empty[Defn]
    val collector = new BlockTraverser:
      override def applyDefn(defn: Defn): Unit = defns += defn
    collector.applyBlock(b)

    val classes = HashMap.empty[Symbol, ClsLikeDefn]
    defns.foreach:
      case cls: ClsLikeDefn if isGeneratedLambdaClass(cls) =>
        classes.addOne(cls.sym -> cls)
        classes.addOne(cls.isym -> cls)
      case _ => ()

    def lambdaClass(path: Path): Opt[ClsLikeDefn] = path match
      case Value.SimpleRef(sym) => classes.get(sym)
      case Value.MemberRef(sym, disamb) => classes.get(sym).orElse(classes.get(disamb))
      case _ => N

    val privateFunctions = HashMap.empty[Symbol, FunDefn]
    defns.foreach:
      case fun: FunDefn if !fun.sym.nameIsMeaningful =>
        privateFunctions.addOne(fun.sym -> fun)
        privateFunctions.addOne(fun.dSym -> fun)
      case _ => ()

    privateFunctions.valuesIterator.toSet.foreach: fun =>
        val instantiations = ListBuffer.empty[Instantiate -> ClsLikeDefn]
        val instantiationCollector = new BlockTraverser:
          override def applyDefn(defn: Defn): Unit = ()
          override def applyResult(result: Result): Unit = result match
            case inst: Instantiate =>
              lambdaClass(inst.cls).foreach(cls => instantiations += inst -> cls)
              super.applyResult(result)
            case _ => super.applyResult(result)
        instantiationCollector.applyBlock(fun.body)
        instantiations.toList match
          case (_, cls) :: Nil =>
            val ctor = GeneratedLambdaCtor(fun, cls)
            generatedLambdaCtors.addOne(fun.sym -> ctor)
            generatedLambdaCtors.addOne(fun.dSym -> ctor)
          case _ => ()

    defns.foreach:
      case cls: ClsLikeDefn => cls.companion.filter(_.isStaged).foreach: companion =>
        val usedClasses = HashSet.empty[ClsLikeDefn]
        val usedFunctions = HashSet.empty[FunDefn]
        val seenFunctions = HashSet.empty[BlockMemberSymbol]

        def collectClass(cls: ClsLikeDefn): Unit =
          if usedClasses.add(cls) then collectReferences(_.applyClsLikeDefn(cls))

        def collectFunction(fun: FunDefn): Unit =
          if seenFunctions.add(fun.sym) then
            usedFunctions += fun
            collectReferences(_.applyBlock(fun.body))

        def collectReferences(apply: BlockTraverser => Unit): Unit =
          val references = new BlockTraverser:
            override def applySymbol(sym: Symbol): Unit =
              generatedLambdaCtors.get(sym).foreach: ctor =>
                collectClass(ctor.cls)
                collectFunction(ctor.fun)
              classes.get(sym).foreach(collectClass)
              privateFunctions.get(sym).foreach(collectFunction)
          apply(references)

        collectReferences(_.applyCompanionModule(companion))
        usedClasses.foreach: cls =>
          stagedLambdaClasses.addOne(cls.sym -> cls)
          stagedLambdaClasses.addOne(cls.isym -> cls)
        val lambdaFunctions = usedFunctions.toList.sortBy(_.sym.uid)
        lambdaFunctions.foreach: fun =>
          val name = stagedLambdaFunctionNames.getOrElseUpdate(fun.sym, scope.allocateOrGetName(fun.sym))
          stagedLambdaFunctionNames.addOne(fun.dSym -> name)
          def recordOwner(sym: Symbol): Unit =
            stagedLambdaFunctionOwners.get(sym).foreach: owner =>
              softAssert(owner == companion.isym, s"Generated lambda helper `${fun.sym.nme}` belongs to multiple staged modules")
            stagedLambdaFunctionOwners.addOne(sym -> companion.isym)
          recordOwner(fun.sym)
          recordOwner(fun.dSym)
        stagedModuleLambdaFunctions.addOne(companion.isym -> lambdaFunctions.map(markStaged))
      case _ => ()

  def apply(prog: Program) =
    collectStagedLambdaClasses(prog.main)
    mkDefnMap(prog.main)
    applyProgram(prog)
