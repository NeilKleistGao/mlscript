    ...
    case Define(defn: ClsLikeDefn, rest) if defn.isStaged =>
      ...
      def replaceSuper(parentPath: Path) = new BlockTransformer(SymbolSubst.Id):
        override def applyResult(r: Result)(k: Result => Block) = super.applyResult(r):
          case Call(Value.SimpleRef(sym: BuiltinSymbol), args) if sym.nme == "super" =>
            k(Call(parentPath, args)(true, false, false))
          case r => k(r)
      ...
      val newModule = defn.copy(sym = sym, companion = S(newCompanion))(
        defn.configOverride,
        defn.annotations.filter:
