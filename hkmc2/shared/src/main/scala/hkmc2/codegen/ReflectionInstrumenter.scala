...
class ReflectionInstrumenter(using State, Raise, Ctx) extends BlockTransformer(SymbolSubst.Id):
              ...
              transformParamsOpt(paramsOpt): (paramsOpt, ctx) =>
                auxParams.map(ps => ctx => transformParamList(ps)(using ctx)).chainContext: (auxParams, ctx) =>
                  tuple(auxParams): auxParams =>
                    blockCtor("ConcreteClassSymbol", Ls(toValue(newName), path, paramsOpt, auxParams), symName)(checkMap("classMap", path, _, ctx))
            case _: ModuleOrObjectSymbol =>
              blockCtor("ModuleSymbol", Ls(toValue(newName), path), symName)(checkMap("moduleMap", path, _, stagingCtx))
        case _ =>
  ...
  def stageMethods(ownerSym: DefinitionSymbol[?], modSym: InnerSymbol, forClass: Bool, cacheNme: Str, generatorMapNme: Str)(methods: Ls[FunDefn]): (FunDefn, Ls[FunDefn], Block => Block) =
    ...
    val previousStageValues = 
      scope.getBindings.collect({
        case b @ (m: ModuleOrObjectSymbol, _) if m != State.unitSymbol => b
        case b @ (c: ClassSymbol, _) if !ctx.builtins.virtualClasses(c) => b
      }).map((key, nme) => 
        val tsym = TermSymbol(syntax.ImmutVal, S(modSym), Tree.Ident(nme))
        val sym = BlockMemberSymbol(nme, Nil)
        
        val goodKey = key.asInstanceOf[DefinitionSymbol[ModuleOrObjectDef | ClassDef]]
        val defn = goodKey.defn.orElse(defnMap.get(goodKey)).getOrElse(throw new Exception(s"Cannot find definition of ${goodKey}"))
        val bsym = defn match
          case l: ModuleOrObjectDef => l.bsym
          case l: ClassDef => l.bsym
          case l: ClsLikeDefn => l.sym
        val owner = defn match
          case l: (ModuleOrObjectDef | ClassDef) => l.owner.map(_.asPath) // is this sufficient? what to do for nested modules?
          case l: ClsLikeDefn => l.owner.map(_.asPath)
        val path = owner match
          case S(owner) => owner.sel(Tree.Ident(bsym.nme), goodKey)
          case N => Value.Ref(bsym, S(goodKey))

        (tsym, sym, path)
      )

    def previousStageDecl(b: Block) =
      previousStageValues.foldRight(b)({case ((tsym, sym, key), acc) =>
