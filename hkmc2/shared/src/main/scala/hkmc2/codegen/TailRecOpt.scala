...
class TailRecOpt(using State, TL, Raise):
  ...
  private def rewriteCallArgs(f: FunDefn, c: Call): CallArgsResult =
    ...
    if c.argss.length =/= f.params.length then return CallArgsResult.Failure
      ...
      case (S(acc), (head, cArgs)) =>
        ...
        val hd = for a <- headArgs yield a.spread match
          case Some(SpreadKind.Eager) =>
            hasSpread = true
            a.value
          case Some(SpreadKind.Lazy) => lastWords("Lazy spread in arguments")
          case _ => a.value
        ...
        if head.restParam.isDefined then
          ...
          S(acc ::: hd)
    allResults match
      case S(res) => CallArgsResult.Success(res)
      case N => CallArgsResult.Failure
      ...
      override def applyBlock(b: Block): Block = b match
        case TailCallShape(calleeSym, c) => dSymIds.get(calleeSym) match
          case Some(id) =>
            ...
            rewriteCallArgs(callee, c) match
              ...
              case CallArgsResult.ForceSpread =>
                ...
                assert(callee.params.size == 1)
                val argList = callee.params.head
                val paramList = argList.params
                val restParam = argList.restParam
                
                val tupleSym = TempSymbol(N, "argList")
                val tupleRes = Tuple(false, c.argss.flatten)
                // Main args
