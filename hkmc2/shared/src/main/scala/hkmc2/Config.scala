...
object ConfigParser:
  ...
  private def parseField(name: Str, value: Tree)(using Raise): Config => Config = name match
    ...
    // Compatibility with generated staging output from before optimizer settings were grouped.
    case "disableDataFlowAnalysis" =>
      parsedField(value)(parseBool)(v => _.mapOptimizer(_.copy(dataFlowAnalysis = !v)))
    case _ =>
      raise(ErrorReport(
