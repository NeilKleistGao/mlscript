...
object Main {
  ...
  val etl = new utils.TraceLogger:
    override protected[hkmc2] def emitDbg(str: Str): Unit = output(Stage.Elaborator, str)
    override def doTrace: Bool = debugElaboration
  val rtl = new utils.TraceLogger:
    override protected[hkmc2] def emitDbg(str: Str): Unit = output(Stage.Resolver, str)
    override def doTrace: Bool = debugResolving
  val ltl = new utils.TraceLogger:
    override protected[hkmc2] def emitDbg(str: Str): Unit = output(Stage.Lowering, str)
    override def doTrace: Bool = debugTyping
  val jtl = new utils.TraceLogger:
    override protected[hkmc2] def emitDbg(str: Str): Unit = output(Stage.Generation, str)
  ...
  def compile(source: String, options: js.Dynamic): js.Dynamic = {
    ...
    val sourceFile = rootPath / "source.mls"
    val origin = Origin(sourceFile, 1, new FastParseHelpers(source))
    ...
    given cctx: CompilerCtx = CompilerCtx.fresh(new io.InMemoryFileSystem(WebImporter.embeddedFiles))
    
    val baseScp: utils.Scope = utils.Scope.empty(utils.Scope.Cfg.default)
    ...
    given Scope = Scope.empty(utils.Scope.Cfg.default)
    given btl: TraceLogger = new utils.TraceLogger:
      override protected[hkmc2] def emitDbg(str: Str): Unit = output(Stage.Typing, str)
      override def doTrace: Bool = true
    def typePurely(term: semantics.Term)(using Ctx): invalml.GeneralType =
      given invalml.InvalCtx = invalml.InvalCtx.init(raise(Stage.Typing))
      InvalTyper().typePurely(term)
    
    def importFile(filePath: Str, moduleName: Str, source: Str, verbose: Bool)(using Config): Unit =
      val file =
        if filePath.startsWith("/") then io.Path(filePath)
        else rootPath / filePath
      ...
      try
        ...
        given Elaborator.Ctx = curCtx.nestLocal(s"import:$moduleName")
        given CompilerCtx = cctx.derive(file)
        given Raise = raise(Stage.Elaborator)
        val elab = new WebImporter(etl, file.up, curCtx)
        ...
        val resolver =
          given Elaborator.Ctx = ctxWithImports
          given Raise = raise(Stage.Resolver)
          semantics.Resolver(rtl)
        curICtx = resolver.traverseBlock(e)(using curICtx)
        typePurely(e)(using ctxWithImports)
      catch
        case err: Throwable =>
          println(err)
    ...
    val (e, newCtx) =
      given Ctx = curCtx.nestLocal("source")
      given CompilerCtx = cctx.derive(sourceFile)
      given Raise = raise(Stage.Elaborator)
      val elab = new WebImporter(etl, sourceFile.up, curCtx)
      elab.topLevel(blk)
  ...
  def report(diag: Diagnostic): Str =
    ...
    val headStr = diag match
      ...
      case InternalError(mainMsg, allMsgs, mkExtraInfo, src) => 
