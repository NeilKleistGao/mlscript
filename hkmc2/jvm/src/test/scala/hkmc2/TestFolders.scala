...
object TestFolders:
  ...
  def compilerPaths(wd: os.Path): MLsCompiler.Paths =
    ...
    new MLsCompiler.Paths:
      ...
      val blockFile: io.Path = compileTestDir(wd)/"Block.mjs"
      val specializeHelpersFile: io.Path = compileTestDir(wd)/"SpecializeHelpers.mjs"
      val optionFile: io.Path = compileTestDir(wd)/"Option.mjs"
      val shapeSetFile: io.Path = compileTestDir(wd)/"ShapeSet.mjs"
